...
#>
    ...
    process {
        ...
        foreach ($record in $InputObject) {
            ...
            if ($_mailFrom) { [string]$mailFrom = [string]::Join(",", (($_mailFrom.trim() -Replace '^<', '') | ForEach-Object { $_.split('>')[0] } | Select-Object -Unique)) } else { [string]$mailFrom = "" }
            ...
            if ($_rcptTo) { [string]$rcptTo = [string]::Join(",", (($_rcptTo.trim() -Replace '^<', '') | ForEach-Object { $_.split('>')[0] })) } else { [string]$rcptTo = "" }
