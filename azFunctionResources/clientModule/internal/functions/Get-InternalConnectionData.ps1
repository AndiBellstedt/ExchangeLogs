...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'ExchangeLogs.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'ExchangeLogs.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'ExchangeLogs.Client.ProtectedToken'
